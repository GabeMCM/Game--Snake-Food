...
export const AdSlot: React.FC<AdSlotProps> = ({ type, onAction }) => {
    ...
    return (
      <button
        className={`glass neon-border flex items-center justify-center gap-3 w-full max-w-xs mx-auto 
                   ${SF_UI.rounding.card} px-6 py-3 cursor-pointer transition-all duration-300
  ...
  return (
    <div className={`glass neon-border flex flex-col ${SF_UI.rounding.card} p-3 mx-auto w-[90%] max-w-sm`}>
      <span className="text-[0.6rem] text-white/40 tracking-widest mb-2">PATROCINADO</span>
      <div className="flex justify-between items-center">
        <div className="flex items-center gap-3">
          <div className={`text-2xl bg-white/10 p-2 ${SF_UI.rounding.inner}`}>🚀</div>
          <div>
        ...
        <button className={`bg-gradient-to-br from-cyan-400 to-blue-400 border-none ${SF_UI.rounding.button} text-white px-4 py-2 text-xs font-bold cursor-pointer hover:scale-105 transition-transform`}>
          VISITAR
        </button>
      </div>
    </div>
